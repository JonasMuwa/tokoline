		...
		<table class="table table-striped" border="1">
			...
			<tbody>
				<?php
					...
					$totalOngkir = 0;
					$no = 0;
					$q = mysqli_query($conn, "Select pesanan.* from pesanan order by id desc") or die (mysql_error());
					while ($data = mysqli_fetch_object($q)) {
						$totalHarga = 0;
						$no++;
						$q2 = mysqli_query($conn, "Select detail_pesanan.*, produk.harga from detail_pesanan INNER JOIN produk ON detail_pesanan.produk_id = produk.id where pesanan_id = '$data->id'") or die (mysql_error());
