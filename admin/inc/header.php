...
<html lang="en">
  <head>
    <meta charset="utf-8">    
		<title>Selamat Dtang Admin</title>
  ...
  $qpesanan = mysqli_query($conn, "select*from pesanan where `read`='0'") or die (mysql_error());
  $totalUnRead = mysqli_num_rows($qpesanan);
  // TOTAL PEMBAYARAN YANG BELUM DI VERIFIKASI
  $qPembayaran = mysqli_query($conn, "select*from pembayaran where `status`='pending'") or die (mysql_error());
  $totalPending = mysqli_num_rows($qPembayaran);
  ?>
    <!-- Fixed navbar -->
    <nav class="navbar navbar-default navbar-fixed-top navbar-blue" style="background: #FF869E">
