		...
		<?php
			if(!empty($_GET)){
				if ($_GET['act'] == "info") {
					...
					if (!empty($_POST)) {
						extract($_POST);
						$query = mysqli_query($conn, "Update info_pembayaran set info='$info' where id='1'") or die (mysql_error());
						if ($query) {
				...
				if ($_GET['act'] == "verified") {
					...
					if ($updatePembayaran) {
						$qPembayaran = mysqli_query($conn, "Select * from pembayaran where id_pesanan='$id_pesanan' and status='verified'") or die (mysql_error());
						$totalPembayaran = 0;
						...
						$q3 = mysqli_query($conn, "select*from pesanan where id='$id_pesanan'") or die (mysql_error());
						$pesanan = mysqli_fetch_object($q3);

						$totalBayar += $pesanan->ongkir;
						if ($totalBayar <= $totalPembayaran) {
							$updatePesanan = mysqli_query($conn, "update pesanan set status='lunas' where id='$id_pesanan'") or die (mysql_error());
							if ($updatePesanan) {
